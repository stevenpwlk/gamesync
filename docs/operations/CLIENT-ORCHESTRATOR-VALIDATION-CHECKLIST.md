...
- [ ] Vérifier un redémarrage Windows pendant une session `InGame`, puis la reprise de capture après stabilité. **Non exécuté — accepté explicitement sans test par l'utilisateur le 10/08/2026**, au vu du volume de preuves réelles déjà accumulées ce soir (dont une vraie interruption/reprise non planifiée, voir plus haut). Reste un point de vigilance pour un incident futur en conditions réelles, pas une case cochée avec preuve.
- [ ] Vérifier une coupure contrôlée pendant l'import et la réconciliation sans nouvelle écriture ambiguë. **Même décision** — la reprise après incident a été prouvée réellement (capture/publication, pas l'import), le mécanisme sous-jacent (`Interrupted`/reprise explicite) est le même ; le cas précis « coupure pendant l'écriture WGS elle-même » reste non exercé en conditions réelles.
...
**Clôture du 10 août 2026 :** toutes les validations Bob sont faites avec preuves réelles (installation, configuration initiale, cycle complet dans les deux sens, incident réel et reprise, remplacement du monde en production). Les deux lignes « redémarrage Windows » et « coupure pendant l'import » de la section précédente restent explicitement non testées ; **l'utilisateur a décidé consciemment de clore le Lot 2 sans ces deux tests**, au vu du volume de preuves réelles déjà réunies ce soir. Ce n'est pas un critère de clôture atteint au sens strict de la phrase ci-dessus, mais un arbitrage explicite de l'utilisateur, à respecter comme tel — ne pas réécrire ces deux lignes comme si elles avaient été prouvées.
